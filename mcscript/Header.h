...
public:
	...
	virtual void swingFrom(const Entity& booster, const std::vector<Block>& blocksInWorld) = 0;
...
public:
	...
	void swingFrom(const Entity& booster, const std::vector<Block>& blocksInWorld) override
	{
		Tnt origin(booster);
			...
			for (Entity* entity : entitiesInWorld) {
				...
				if (tntEntity && tntEntity->isAlive() && tntEntity->getFuse() == 0) {
					...
					for (Entity* otherEntity : entitiesInWorld) {
						if (otherEntity != tntEntity) {
							// Performs explosions functions on other entities
							otherEntity->swingFrom(*tntEntity, blocksInWorld);
						}
					}
					tntEntity->kill();
				}
